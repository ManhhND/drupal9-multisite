...
add_new_site_drush_alias() {
  ...
  if file_ends_with_newline $DRUSH_ALIAS_PATH/rxp.site.yml; then
    echo "$NEW_SITE_NAME:
 root: /app/web
 uri: http://$NEW_SITE_NAME.lndo.site" >>$DRUSH_ALIAS_PATH/rxp.site.yml
  else
    echo "\n$NEW_SITE_NAME:
 root: /app/web
 uri: http://$NEW_SITE_NAME.lndo.site" >>$DRUSH_ALIAS_PATH/rxp.site.yml
...
site_install() {
  ...
  lando drush @rxp.$NEW_SITE_NAME site:install --db-url=mysql://user:user@$NEW_SITE_NAME/$NEW_SITE_NAME --site-name=$NEW_SITE_NAME --locale=$NEW_SITE_LOCALE -y
}
